...
bool DecryptVacModule(uint8_t* base,
	size_t   imgSize,
	const uint8_t key[8]);
...
void FixVacModule(DWORD pImage, DWORD pModule_) {
	...
	for (size_t i = 0; i < pNtHeader->FileHeader.NumberOfSections; i++)
	{
		...
		printf(("[+] : %p .%s\t(size : %x)"), i, &pSectionHeader->Name[1], pSectionHeader->PointerToRawData, pSectionHeader->Misc.VirtualSize);

		if (!strcmp((char*)&pSectionHeader->Name[0], ".text")) {

			printf(("[+] : EntryPoint fixed to %p"), pSectionHeader->PointerToRawData);
...
bool DecryptVacModule(uint8_t* base,
	...
	const uint8_t key[8])
{
	// 1) obtém a tabela IceChunk apontada por DataDirectory[13]
	...
	// 2) cria a chave ICE (n = 1 → 64 bits)
	IceKey ice(1);
	ice.set(key);                    // primária == secundária já decifrada

	// 3) percorre cada chunk e descriptografa em CBC (IV=0)
	for (size_t i = 0; i < rows; ++i) {
		...
		for (uint32_t off = 0; off < size; off += 8)
			ice.decrypt(&ptr[off], &ptr[off]);       // in-place
	}
	return true;
}

bool DumpVacModule(VacModuleInfo_t* m)
{
	BYTE* base = nullptr; DWORD sz = 0;

	/* 1. descobre endereço e tamanho ----------------------------------*/
	if (m->m_pModule) {                         // manual-map
		base = (BYTE*)m->m_pModule->m_pModuleBase;
		sz = GetAllocationSize((DWORD)base);
	}
	else if (m->m_hModule) {                  // LoadLibrary
		base = (BYTE*)m->m_hModule;
		sz = GetAllocationSize((DWORD)base);
	}
	else if (m->m_pRawModule && m->m_nModuleSize) {               // buffer descriptografado
		base = (BYTE*)m->m_pRawModule;
		sz = m->m_nModuleSize;
	}
	else {
		puts("[-] DumpVacModuleRaw: buffer inexistente");
		return false;
	}
	/* 2. copia bytes exatamente como estão na memória -----------------*/
	...
	char path[MAX_PATH];
	sprintf_s(path, "C:\\Lumina\\vac%08X.dll", m->m_unCRC32);

	std::ofstream fout(path, std::ios::binary);
	fout.write(reinterpret_cast<char*>(buf.data()), buf.size());
	fout.close();

	printf("[+] Raw dump %08X salvo (%u bytes)\n", m->m_unCRC32, sz);
	return true;
}
